 ...
 */
//修改为你的实体类的包名
packageName = "com.sstir.sstirwxoptlog.entity;"
typeMapping = [
        (~/(?i)int/)                      : "Long",
...
def generate(out, table, className, fields) {
  ...
  fields.each() {
    ...
    //if (it.colum != it.name) {
      out.println "\t@Column(name = \"${it.colum}\", columnDefinition = \"${it.comment}\")"
    //}
...
def calcFields(table) {
  DasUtil.getColumns(table).reduce([]) { fields, col ->
    ...
    fields += [[
                       ...
                       colum: col.getName(),
                       type : typeStr,
                       comment: col.getDataType().getSpecification() + " COMMENT '" + col.getComment() + "'",
                       annos: ""]]
  }
}
